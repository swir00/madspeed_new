...
class BLEService with ChangeNotifier {
  ...
  // --- NOWE ZMIENNE DO ZARZĄDZANIA TIMEREM ---
  final ValueNotifier<Duration> trainingDuration = ValueNotifier(Duration.zero);
  Timer? _trainingTimer;
  // Przechowuje poprzedni stan, aby wykryć zmianę isLoggingActive
  GPSData _previousGpsData = GPSData();
    ...
    _currentDataSubscription = _currentDataCharacteristic!.lastValueStream.listen((value) {
      if (value.isNotEmpty) {
        try {
          ...
          final newGpsData = GPSData.fromJson(data);
          _handleTrainingTimer(newGpsData); // Uruchom logikę timera
          // debugPrint("Parsed GPSData - Current Speed: ${_currentGpsData.currentSpeed}"); // Opcjonalnie odkomentuj dla debugowania
          _currentGpsData = newGpsData;
          notifyListeners();
        } catch (e) {
  ...
  // --- NOWE METODY DO OBSŁUGI TIMERA ---
  void _handleTrainingTimer(GPSData newData) {
    final bool wasLogging = _previousGpsData.isLoggingActive ?? false;
    final bool isLogging = newData.isLoggingActive ?? false;

    if (isLogging && !wasLogging) {
      // Logowanie właśnie się rozpoczęło.
      _startTrainingTimer();
    } else if (!isLogging && wasLogging) {
      // Logowanie właśnie się zakończyło.
      _stopTrainingTimer();
    }

    _previousGpsData = newData;
  }

  void _startTrainingTimer() {
    _trainingTimer?.cancel();
    trainingDuration.value = Duration.zero;
    _trainingTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      trainingDuration.value = Duration(seconds: trainingDuration.value.inSeconds + 1);
    });
    debugPrint("Globalny timer treningu uruchomiony.");
  }

  void _stopTrainingTimer() {
    _trainingTimer?.cancel();
    debugPrint("Globalny timer treningu zatrzymany. Czas: ${trainingDuration.value}");
  }

  void _resetTrainingTimer() {
    _trainingTimer?.cancel();
    trainingDuration.value = Duration.zero;
    debugPrint("Globalny timer treningu zresetowany.");
  }
  // --- KONIEC NOWYCH METOD ---
  ...
  Future<void> sendControlCommand(String command) async {
    ...
    // Jeśli wysyłamy komendę RESET, zresetuj również nasz timer w aplikacji
    if (command == "RESET") {
      _resetTrainingTimer();
    }
    try {
  ...
  void dispose() {
    ...
    _trainingTimer?.cancel(); // Anuluj timer przy zamykaniu serwisu
